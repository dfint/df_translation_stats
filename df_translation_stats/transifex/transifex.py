...
async def get_translation_stats(client: httpx.AsyncClient) -> TranslationStats:
    ...
    response = await client.get(url, params={"filter[project]": TX_PROJECT_ID}, headers=headers)
...
async def get_resource_strings_tagged_notranslate(client: httpx.AsyncClient, resource: str) -> list[dict[str, Any]]:
    if not settings.tx_token:
